...
const DashboardAlbum = () => {
  ...
  return (
    <div className="w-full p-4 flex items-center justify-center flex-col">
      <div className="music-w border-2 border-gray-300 rounded-md py-12 px-9 shadow-lg">
        {allAlbums &&
        ...
        >
          ...
          >
            <div className="bg-red-300 px-3 rounded-md">
              <p className="text-headingColor text-sm">{t('yes')}</p>
            </div>
            <div
              className="bg-green-300 px-3 rounded-md"
              onClick={() => setIsDelete(false)}
            >
              <p className="text-headingColor text-sm">{t('no')}</p>
            </div>
          </div>
        </motion.div>
      )}
